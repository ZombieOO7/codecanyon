...
switch ($_POST['action']) {
	...
	if($action=='enable'){
		...
		else if($table=='tbl_sub_category')
		{
			$sql="SELECT * FROM tbl_sub_category WHERE id IN ($ids)";
			$result=mysqli_query($mysqli,$sql);
			while($row=mysqli_fetch_assoc($result))
			{
			...
			$deleteSql="DELETE FROM tbl_sub_category WHERE `cid` IN ($ids)";
			mysqli_query($mysqli, $deleteSql);

			$response['status']=1;
		}
		// over code updated by arjann006@gmail.com 08/01/2022
	...
	case 'get_sub_cat':{
		$cat_id = $_POST["cat_id"];
		$sql="SELECT * FROM tbl_sub_category WHERE category_id='$cat_id'";
		$result=mysqli_query($mysqli,$sql);
		$categoryData = [];
