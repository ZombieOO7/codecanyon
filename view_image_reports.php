<?php 
	...
	$page_title=ucfirst($type)." Status Reports";
	...
	$row=mysqli_fetch_assoc($res);
 	...
 	$sql1="SELECT tbl_reports.*, tbl_users.`name`, tbl_users.`user_image` FROM tbl_reports, tbl_users WHERE tbl_reports.`post_id`='$id' AND tbl_reports.`report_type`='$type' AND tbl_users.`id`=tbl_reports.`user_id` ORDER BY tbl_reports.`report_on` DESC";
	...
	<div class="app-messaging" id="collapseMessaging">
		...
		<div class="heading" style="font-size: 16px">Status Description</div>
			<ul class="group full-height">
				...
				<li class="message">
					<a href="view_image_comments.php?post_id=<?=$id?>&type=<?=$type?>">
					<div class="message">
			...
			<div class="heading">
				<div class="title" style="font-size: 16px">
					...
					<strong>Title: </strong>&nbsp;&nbsp;<?php
		                if(strlen($row['image_title']) > 80){
		                  echo substr(stripslashes($row['image_title']), 0, 80).'...';  
		                }else{
