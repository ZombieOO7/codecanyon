...
function user_total_status($user_id, $is_other_profile = false)
{
    global $mysqli;
    ...
    $sql = "SELECT COUNT(*) as num FROM tbl_video WHERE `user_id`='$user_id' $StatusWhere";
    $res = mysqli_query($mysqli, $sql);
    $row = mysqli_fetch_array($res);
    $total_count += $row['num'];
    mysqli_free_result($res);

    $sql = "SELECT COUNT(*) as num FROM tbl_img_status WHERE `user_id`='$user_id' $StatusWhere";
    $res = mysqli_query($mysqli, $sql);
    $row = mysqli_fetch_array($res);
    $total_count += $row['num'];
    mysqli_free_result($res);

    $sql = "SELECT COUNT(*) as num FROM tbl_quotes WHERE `user_id`='$user_id' $StatusWhere";
    $res = mysqli_query($mysqli, $sql);
    $row = mysqli_fetch_array($res);
    $total_count += $row['num'];
    mysqli_free_result($res);
...
if ($get_method['method_name'] == "home") {
    ...
    if (isset($get_method['page']) && $get_method['page'] != '') {
        $page = $get_method['page'];
    } else {
        $page = 1;
    }
    ...
    if ($page == 1) {
        ...
        $sql = "SELECT * FROM tbl_slider WHERE `status`='1' ORDER BY `id` DESC";
        ...
        while ($data = mysqli_fetch_assoc($result)) {
            ...
            $data_arr['id'] = $data['post_id'];
            $data_arr['status_type'] = $data['slider_type'];
            $data_arr['status_title'] = $slider_title;
            $data_arr['status_layout'] = $layout;

            if ($data['slider_type'] != 'quote') {
                $data_arr['status_thumbnail_b'] = $file_path . 'images/' . $image;

                if ($layout == 'Portrait') {
                    $data_arr['status_thumbnail_s'] = get_thumb('images/' . $image, '280x500');
                } else {
                    $data_arr['status_thumbnail_s'] = get_thumb('images/' . $image, '500x280');
                }

                $data_arr['quote_bg'] = '';
                $data_arr['quote_font'] = '';
            } else {
                $data_arr['status_thumbnail_b'] = '';
                $data_arr['status_thumbnail_s'] = '';
                $data_arr['quote_bg'] = '#' . get_single_info($data['post_id'], 'quote_bg', 'quote');
                $data_arr['quote_font'] = get_single_info($data['post_id'], 'quote_font', 'quote');
            }

            $data_arr['external_link'] = ($data['external_url'] != '') ? $data['external_url'] : '';
            $data_arr['total_viewer'] = $total_views;
    ...
    if (!empty($lang_ids)) {
		...
		LEFT JOIN tbl_category ON tbl_video.`cat_id`= tbl_category.`cid` 
		WHERE tbl_video.`status`='1'AND tbl_category.`status`='1' AND ($column) ORDER BY tbl_video.`id` $video_order_by LIMIT $limit, $page_limit";
    } else {
        $query = "SELECT * FROM tbl_video
		LEFT JOIN tbl_category ON tbl_video.`cat_id`= tbl_category.`cid` 
    ...
    while ($data = mysqli_fetch_assoc($sql)) {
        ...
        $data_arr['id'] = $data['id'];
        $data_arr['status_type'] = 'video';
        $data_arr['status_title'] = $data['video_title'];
        $data_arr['status_layout'] = $data['video_layout'];

        if ($data['video_type'] == 'server_url' or $data['video_type'] == 'local') {
            $data_arr['status_thumbnail_b'] = $file_path . 'images/' . $data['video_thumbnail'];

            $data_arr['status_thumbnail_s'] = get_thumb('images/' . $data['video_thumbnail'], '500x280');
        } else {
            $data_arr['status_thumbnail_b'] = $data['video_thumbnail'];
            $data_arr['status_thumbnail_s'] = $data['video_thumbnail'];
        }

        $data_arr['total_likes'] = $data['total_likes'];
        $data_arr['total_viewer'] = $data['totel_viewer'];
        ...
        if ($num_rows1 > 0) {
            $data_arr['already_like'] = true;
        } else {
            $data_arr['already_like'] = false;
        }

        $data_arr['is_favourite'] = get_favourite_info($data['id'], $get_method['user_id'], 'video');
    ...
    if (!empty($lang_ids)) {
		...
		LEFT JOIN tbl_category ON tbl_img_status.`cat_id`= tbl_category.`cid` 
		WHERE tbl_img_status.`status`='1' AND tbl_category.`status`='1' AND ($column) ORDER BY tbl_img_status.`id` $video_order_by LIMIT $limit, $page_limit";
    } else {
        $query = "SELECT * FROM tbl_img_status
		LEFT JOIN tbl_category ON tbl_img_status.`cat_id`= tbl_category.`cid` 
		WHERE tbl_img_status.`status`='1' AND tbl_category.`status`='1' ORDER BY tbl_img_status.`id` $video_order_by LIMIT $limit, $page_limit";
    }
    ...
    while ($data = mysqli_fetch_assoc($sql)) {
        ...
        $data_arr['id'] = $data['id'];
        $data_arr['status_type'] = $data['status_type'];
        $data_arr['status_title'] = $data['image_title'];
        $data_arr['status_layout'] = $data['image_layout'];

        $data_arr['status_thumbnail_b'] = $file_path . 'images/' . $data['image_file'];

        $data_arr['status_thumbnail_s'] = get_thumb('images/' . $data['image_file'], '500x280');

        $data_arr['total_likes'] = $data['total_likes'];
        $data_arr['total_viewer'] = $data['total_views'];
        ...
        $data_arr['is_favourite'] = get_favourite_info($data['id'], $get_method['user_id'], $data['status_type']);
    ...
    while ($data = mysqli_fetch_assoc($sql)) {
        ...
        $data_arr['id'] = $data['id'];
        $data_arr['status_type'] = 'quote';
        $data_arr['status_title'] = stripslashes($data['quote']);
        $data_arr['status_layout'] = 'Landscape';
        ...
        $data_arr['is_favourite'] = get_favourite_info($data['id'], $get_method['user_id'], 'quote');

        $data_arr['quote_bg'] = '#' . $data['quote_bg'];
        $data_arr['quote_font'] = $data['quote_font'];

        array_push($jsonObj, $data_arr);
    }

    $row['statuses'] = $jsonObj;
    ...
    if (!empty($lang_ids)) {
        ...
        $query = "SELECT * FROM tbl_video
		LEFT JOIN tbl_category ON tbl_video.`cat_id`=tbl_category.`cid` 
		WHERE tbl_video.`id` <> '$post_id' AND tbl_video.`status`='1' AND tbl_category.`status`='1' AND tbl_video.`cat_id`='$cat_id' AND tbl_video.`sub_cat_id`='$sub_cat_id' AND ($column) ORDER BY tbl_video.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
    } else {
        $query = "SELECT * FROM tbl_video
		LEFT JOIN tbl_category ON tbl_video.`cat_id`=tbl_category.`cid` 
		WHERE tbl_video.`id` <> '$post_id' AND tbl_video.`status`='1' AND tbl_category.`status`='1' AND tbl_video.`cat_id`='$cat_id' AND tbl_video.`sub_cat_id`='$sub_cat_id' ORDER BY tbl_video.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
    }
    ...
    while ($data = mysqli_fetch_assoc($sql_video)) {
        ...
        // 		$row['sub_category_name'] = @$data['sub_category_name']?$data['sub_category_name']:'';
        $row['cat_id'] = $data['cat_id'];
        $row['sub_cat_id'] = @$data['sub_cat_id'] ? $data['sub_cat_id'] : '';
    ...
    if (!empty($lang_ids)) {
        ...
        $query = "SELECT * FROM tbl_img_status
		LEFT JOIN tbl_category ON tbl_img_status.`cat_id`=tbl_category.`cid` 
		WHERE tbl_img_status.`id` <> '$post_id' AND tbl_img_status.`status`='1' AND tbl_category.`status`='1' AND tbl_img_status.`cat_id`='$cat_id' AND tbl_img_status.`sub_cat_id`='$sub_cat_id' AND ($column) ORDER BY tbl_img_status.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
    } else {
        $query = "SELECT * FROM tbl_img_status
		LEFT JOIN tbl_category ON tbl_img_status.`cat_id`=tbl_category.`cid` 
		WHERE tbl_img_status.`id` <> '$post_id' AND tbl_img_status.`status`='1' AND tbl_category.`status`='1' AND tbl_img_status.`cat_id`='$cat_id' AND tbl_img_status.`sub_cat_id`='$sub_cat_id' ORDER BY tbl_img_status.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
    }
    ...
    if ($layout == 'Landscape') {
        ...
        if (!empty($lang_ids)) {
            ...
            $query = "SELECT * FROM tbl_quotes
			LEFT JOIN tbl_category ON tbl_quotes.`cat_id`=tbl_category.`cid` 
			WHERE tbl_quotes.`id` <> '$post_id' AND tbl_quotes.`status`='1' AND tbl_category.`status`='1' AND tbl_quotes.`cat_id`='$cat_id' AND tbl_quotes.`sub_cat_id`='$sub_cat_id' AND ($column) ORDER BY tbl_quotes.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
        } else {

            $query = "SELECT * FROM tbl_quotes
			LEFT JOIN tbl_category ON tbl_quotes.`cat_id`=tbl_category.`cid` 
            WHERE tbl_quotes.`id` <> '$post_id' AND tbl_quotes.`status`='1' AND tbl_category.`status`='1' AND tbl_quotes.`cat_id`='$cat_id' AND tbl_quotes.`sub_cat_id`='$sub_cat_id' ORDER BY tbl_quotes.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
        }
    ...
    $query = "SELECT * FROM tbl_category WHERE tbl_category.`status`='1' ORDER BY tbl_category." . $cat_order . " LIMIT $limit, $page_limit";
    $sql = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
    ...
    if (!empty($lang_ids)) {
        ...
        $query = "SELECT * FROM tbl_video
		LEFT JOIN tbl_category ON tbl_video.`cat_id`=tbl_category.`cid` 
		WHERE tbl_video.`status`='1' AND tbl_category.`status`='1' AND tbl_video.`cat_id`='$cat_id' AND tbl_video.`sub_cat_id`='$sub_cat_id' AND ($column) ORDER BY tbl_video.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
    } else {
        $query = "SELECT * FROM tbl_video
		LEFT JOIN tbl_category ON tbl_video.`cat_id`=tbl_category.`cid` 
        ...
        case 'video': 
            {
                ...
                if ($res->num_rows > 0) {
                    ...
                    $set['total_comment'] = CountRow('tbl_comments', "post_id='" . $status_id . "' AND type='video'");

                    $set['id'] = $data['id'];
                    $set['cat_id'] = $data['cat_id'];
                    $set['sub_cat_id'] = $data['sub_cat_id'];
                    $set['status_type'] = 'video';
                    $set['status_title'] = $data['video_title'];
                    $set['status_layout'] = $data['video_layout'];
                    $set['video_url'] = $video_file;

                    $set['status_thumbnail_b'] = $file_path . 'images/' . $data['video_thumbnail'];

                    if ($data['video_layout'] == 'Landscape') {
                        $set['status_thumbnail_s'] = get_thumb('images/' . $data['video_thumbnail'], '500x280');
                    } else if ($data['video_layout'] == 'Portrait') {
                        $set['status_thumbnail_s'] = get_thumb('images/' . $data['video_thumbnail'], '280x500');
                    }

                    $set['total_likes'] = $data['total_likes'];
                    $set['total_viewer'] = $data['totel_viewer'];
                    $set['total_download'] = $data['total_download'];

                    $set['category_name'] = $data['category_name'];
                    // $set['sub_category_name'] = $data['sub_category_name'];
                    $set['cat_id'] = $data['cat_id'];
                    $set['sub_cat_id'] = (@$data['sub_cat_id'] != null) ? $data['sub_cat_id'] : '';

                    if ($data['created_at'] != 0) {
                        $set['created_at'] = calculate_time_span($data['created_at']);
                    } else {
                        $set['created_at'] = '';
                    }
                    ...
                    if ($num_like > 0) {
                        $set['already_like'] = true;
                    } else {
                        $set['already_like'] = false;
                    }

                    $set['is_favourite'] = get_favourite_info($data['id'], $get_method['user_id'], 'video');

                    $set['watermark_image'] = $settings_details['watermark_image'] ? $file_path . 'images/' . $settings_details['watermark_image'] : "";

                    if ($set['watermark_image'] == '') {
                        $set['watermark_on_off'] = 'false';
                    } else {
                        $set['watermark_on_off'] = $settings_details['watermark_on_off'];
                    }

                    $set['user_id'] = $data['user_id'];
                    $set['user_name'] = get_user_info($data['user_id'], 'name');

                    if (get_user_info($data['user_id'], 'user_image') != '') {
                        $set['user_image'] = $file_path . 'images/' . get_user_info($data['user_id'], 'user_image');
                    } else {
                        $set['user_image'] = '';
                    }
                    ...
                    $sql_comment = "SELECT * FROM tbl_comments WHERE `post_id`='" . $status_id . "' AND `status`='1' AND `type`='video' ORDER BY `id` DESC LIMIT 1";
                    ...
                    if ($res->num_rows > 0) {
                        ...
                        $row_comment['post_id'] = '';
                        $row_comment['status_type'] = '';
                        $row_comment['comment_text'] = '';
                        $row_comment['comment_date'] = '';
                    }

                    $set['user_comments'] = $row_comment;
                } else {
                    $success = 0;
                }
            }
        ...
        case 'image': 
            {
                ...
                if ($res->num_rows > 0) {
                    ...
                    $set['total_comment'] = CountRow('tbl_comments', "post_id='" . $status_id . "' AND type='" . $type . "'");

                    $set['id'] = $data['id'];
                    $set['cat_id'] = $data['cat_id'];
                    $set['sub_cat_id'] = $data['sub_cat_id'];
                    $set['status_type'] = $data['status_type'];
                    $set['status_title'] = $data['image_title'];
                    $set['status_layout'] = $data['image_layout'];
                    $set['video_url'] = '';

                    $set['status_thumbnail_b'] = $file_path . 'images/' . $data['image_file'];

                    if ($data['image_layout'] == 'Landscape') {
                        $set['status_thumbnail_s'] = get_thumb('images/' . $data['image_file'], '500x280');
                    } else if ($data['image_layout'] == 'Portrait') {
                        $set['status_thumbnail_s'] = get_thumb('images/' . $data['image_file'], '280x500');
                    }

                    $set['total_likes'] = $data['total_likes'];
                    $set['total_viewer'] = $data['total_views'];
                    $set['total_download'] = $data['total_download'];
                    ...
                    $set['is_favourite'] = get_favourite_info($data['id'], $get_method['user_id'], $type);

                    $set['watermark_image'] = '';

                    $set['watermark_on_off'] = '';
                    ...
                    $sql_comment = "SELECT * FROM tbl_comments WHERE `post_id`='" . $status_id . "' AND `status`='1' AND `type`='" . $type . "' ORDER BY id DESC LIMIT 1";
        ...
        case 'quote': {
			...
			WHERE tbl_quotes.`id`='$status_id' AND tbl_category.`status`='1'";
                ...
                if ($res->num_rows > 0) {
                    ...
                    $set['total_comment'] = CountRow('tbl_comments', "post_id='" . $status_id . "' AND type='quote'");

                    $set['id'] = $data['id'];
                    $set['cat_id'] = $data['cat_id'];
                    $set['sub_cat_id'] = $data['sub_cat_id'];
                    $set['status_type'] = 'quote';
                    $set['status_title'] = stripslashes($data['quote']);
                    $set['status_layout'] = 'Landscape';
                    $set['video_url'] = '';

                    $set['status_thumbnail_b'] = '';
                    $set['status_thumbnail_s'] = '';
                    ...
                    $set['quote_bg'] = '#' . $data['quote_bg'];
                    $set['quote_font'] = $data['quote_font'];
                    ...
                    $set['is_favourite'] = get_favourite_info($data['id'], $get_method['user_id'], 'quote');
                    ...
                    // get comments related to image status
                    $sql_comment = "SELECT * FROM tbl_comments WHERE `post_id`='" . $status_id . "' AND `status`='1' AND `type`='quote' ORDER BY id DESC LIMIT 1";
    ...
    $set['video_views_status_ad'] = $settings_details['video_views_status'];

    $set['like_video_status_ad'] = $settings_details['like_video_points_status'];
    $set['download_video_status_ad'] = $settings_details['download_video_points_status'];

    $set['like_image_status_ad'] = $settings_details['like_image_points_status'];
    $set['download_image_status_ad'] = $settings_details['download_image_points_status'];

    $set['like_gif_points_status_ad'] = $settings_details['like_gif_points_status'];
    $set['download_gif_status_ad'] = $settings_details['download_gif_points_status'];

    $set['like_quotes_status_ad'] = $settings_details['like_quotes_points_status'];
    ...
    $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['view_count'], 'success' => 1);
    ...
    switch ($type) {
        case 'video': {
                if (mysqli_num_rows($res_like) == 0) {
                    ...
                    $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['like'], 'success' => '1', 'activity_status' => '1', 'total_likes' => $total_likes);
                } else {
                    ...
                    if ($res_delete) {
                        ...
                        $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['unlike'], 'success' => '1', 'activity_status' => '0', 'total_likes' => $total_likes);
                    } else {
                        $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['unlike_err'], 'success' => '0');
                    }
                }
            }
            break;
    ...
    $set = array('status' => '1', 'message' => '', 'total_download' => $total_download, 'msg' => $app_lang['video_download'], 'success' => 1);
    ...
    if ($uploadStatus) {
        ...
        $set = array('status' => '1', 'message' => '', 'msg' => ucfirst($status_type) . ' ' . $app_lang['upload_success'], 'success' => '1');
    } else {
        $set = array('status' => '1', 'message' => '', 'msg' => 'Error in uploading status', 'success' => '1');
    }
    ...
    $set = array('status' => '1', 'message' => '', 'msg' => 'Quotes ' . $app_lang['upload_success'], 'success' => '1');
    header('Content-Type: application/json; charset=utf-8');
    echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    die();
    ...
    if ($res->num_rows < $upload_limit) {
        $set = array('status' => '1', 'message' => '', 'success' => '1', 'msg' => '');
    } else {
        $set = array('status' => '1', 'message' => '', 'success' => '0', 'msg' => $app_lang['exceed_upload_limit']);
    }
    ...
    if ($row['email'] != "") {
        $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['email_exist'], 'success' => '0');
    } else {
        $to = $email;
        $recipient_name = '';
        // subject
        ...
        $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['otp_sent'], 'success' => '1');
    }
    ...
    if ($user_type == 'Google' || $user_type == 'google') {
        ...
        if ($num_rows == 0) {
            ...
            $set = array('status' => '1', 'message' => '', 'user_id' => strval($user_id), 'name' => $get_method['name'], 'email' => $get_method['email'], 'success' => '1', 'msg' => '', 'auth_id' => $auth_id, 'referral_code' => true);
        } else {
            ...
            if ($row['status'] == 2) {
                ...
                $set = array('status' => '1', 'message' => '', 'user_id' => $row['id'], 'name' => $row['name'], 'email' => $row['email'], 'success' => '1', 'msg' => '', 'auth_id' => $auth_id, 'referral_code' => false);
            }
        }
        ...
        if ($row['email'] != "") {
            ...
            $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['register_success'], 'success' => '1');
        }
    }
    header('Content-Type: application/json; charset=utf-8');
    echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    ...
    if (!isset($get_method['user_id'])) {
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            if (strcmp($row['password'], md5($password)) == 0) {
                if ($row['status'] == 0) {
                    ...
                    $set = array('status' => '1', 'message' => '', 'user_id' => $row['id'], 'name' => $row['name'], 'email' => $row['email'], 'msg' => $app_lang['login_success'], 'auth_id' => '', 'success' => '1');
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            if ($row['status'] == 0) {
                ...
                $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['login_success'], 'success' => '1');
            }
        } else {
            $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['email_not_found'], 'success' => '0');
        }
...
// change password
    ...
    if (strcmp($row['password'], md5($old_password)) == 0) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $set = array('status' => '1', 'message' => '', 'user_id' => $row['id'], 'name' => $row['name'], 'email' => $row['email'], 'phone' => $phone, 'user_image' => $user_image, 'user_youtube' => $user_youtube, 'user_instagram' => $user_instagram, 'user_code' => $row['user_code'], 'total_point' => thousandsNumberFormat(get_total_points($user_id)), 'total_status' => $user_total_status, 'total_followers' => $total_followers, 'total_following' => $total_following, 'is_verified' => $is_verified, 'success' => '1', 'msg' => '');
    } else {
        $set = array('status' => '1', 'message' => '', 'success' => '0', 'msg' => '');
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $set['user_code'] = $row['user_code'];
        $set['total_point'] = get_total_points($user_id);
        $set['pending_point'] = $total_pending;
        $set['total_video'] = $total_video;
        $set['total_image'] = $total_image;
        $set['total_gif'] = $total_gif;
        $set['total_quote'] = $total_quote;
        $set['total_followers'] = strval($total_followers);
        $set['total_following'] = strval($total_following);

        $set['is_verified'] = $is_verified;
        $set['delete_note'] = $settings_details['delete_note'];
    } else {
        $set['success'] = '0';
        $set['msg'] = '';
    }
    ...
    if ($result->num_rows > 0) {
        ...
        $set['success'] = '1';
        $set['msg'] = $app_lang['account_deleted'];
    } else {

        $set['success'] = '0';
        $set['msg'] = $app_lang['error_msg'];
    }
    ...
    if (isset($get_method['is_remove']) && $is_remove) {
        if ($row['user_image'] != "") {
    ...
    if ($get_method['password'] != "") {
        $user_edit = "UPDATE tbl_users SET name='" . $get_method['name'] . "',password='" . md5(trim($get_method['password'])) . "',phone='" . $get_method['phone'] . "',user_image='" . $user_image . "',user_youtube='" . $get_method['user_youtube'] . "',user_instagram='" . $get_method['user_instagram'] . "' WHERE id = '" . $get_method['user_id'] . "'";
    } else {
        $user_edit = "UPDATE tbl_users SET name='" . $get_method['name'] . "',phone='" . $get_method['phone'] . "',user_image='" . $user_image . "',user_youtube='" . $get_method['user_youtube'] . "',user_instagram='" . $get_method['user_instagram'] . "' WHERE id = '" . $get_method['user_id'] . "'";
    }
    ...
    if ($result->num_rows > 0) {
        ...
        $user_total_status = user_total_status($row['id'], true);

        $set = array('status' => '1', 'message' => '', 'user_id' => $row['id'], 'name' => $row['name'], 'email' => $row['email'], 'user_image' => $user_image, 'user_code' => $row['user_code'], 'user_youtube' => $user_youtube, 'user_instagram' => $user_instagram, 'already_follow' => $already_follow, 'total_status' => $user_total_status, 'total_followers' => $total_followers, 'total_following' => $total_following, 'is_verified' => $is_verified, 'success' => '1', 'msg' => '');
    } else {
        $set = array('status' => '1', 'message' => '', 'success' => '0', 'msg' => $app_lang['error_msg']);
    }
    ...
    if ($num_rows > 0) {
        ...
        if ($row['status'] == 0) {
            $set = array('status' => '1', 'message' => '', 'profile_status' => '0', 'profile_message' => 'Request is pending', 'success' => '1', 'name' => $name, 'email' => $email, 'msg' => '');
        } else if ($row['status'] == 2) {
            $set = array('status' => '1', 'message' => '', 'profile_status' => '2', 'profile_message' => 'Request is rejected', 'success' => '1', 'name' => $name, 'email' => $email, 'msg' => '');
        } else {
            $set = array('status' => '1', 'message' => '', 'profile_status' => '1', 'profile_message' => 'Verified', 'success' => '1', 'name' => $name, 'email' => $email, 'msg' => '');
        }
    } else {
        $set = array('status' => '1', 'message' => '', 'profile_status' => '3', 'profile_message' => 'Not verified !', 'success' => '1', 'name' => $name, 'email' => $email, 'msg' => '');
    }
    ...
    if ($num_rows > 0) {
        ...
        $set['id'] = $row['id'];
        $set['user_full_name'] = $row['full_name'];
        $set['user_message'] = $row['message'];
        $set['document_img'] = $file_path . 'images/documents/' . $row['document'];
        $set['request_date'] = date('d-m-Y', $row['created_at']);

        if ($row['verify_at'] != 0) {
            $set['response_date'] = date('d-m-Y', $row['verify_at']);
        } else {
            $set['response_date'] = '';
        }

        if ($row['status'] == 2) {
            $set['admin_message'] = $row['reject_reason'];
        } else {
            $set['admin_message'] = '';
        }

        $set['av_status'] = $row['status'];
    } else {
        $set = array('msg' => 'Not varified !', 'success' => '0');
    }
    ...
    if ($result->num_rows > 0) {
        ...
        if (send_email($to, $recipient_name, $subject, $message)) {
            ...
            $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['password_sent_err'], 'success' => '0');
        }
    } else {
        $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['email_not_found'], 'success' => '0');
    }
    ...
    while ($data = mysqli_fetch_assoc($sql)) {
        ...
        if ($num_rows > 0) {
            while ($data_arr = mysqli_fetch_assoc($res)) {
                ...
                $users_rewards_activity = $row1;

                array_push($jsonObj, $users_rewards_activity);
            }
        }
    }
    ...
    $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['redeem_request_success'], 'success' => 1);
    ...
    if ($res->num_rows == 0) {
        ...
        $set['success'] = 1;
        $set['msg'] = $app_lang['add_favourite'];
        ...
        if (mysqli_query($mysqli, $deleteSql)) {
            ...
            $set['success'] = 1;
            $set['msg'] = $app_lang['remove_favourite'];
    ...
    while ($row_favourite = mysqli_fetch_assoc($res)) {
        ...
        while ($data = mysqli_fetch_assoc($sql_video)) {
            ...
            if ($data['created_at'] != 0) {
                $row['created_at'] = calculate_time_span($data['created_at']);
            } else {
                $row['created_at'] = '';
            }
    ...
    if ($uploadStatus) {
        ...
        $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['upload_video_success'], 'success' => 1);
    } else {
        $set = array('status' => '1', 'message' => '', 'msg' => 'Video upload issue !!', 'success' => 1);
    }
    ...
    $set = array('status' => '1', 'message' => '', 'msg' => $app_lang['status_delete'], 'success' => 1);
    ...
    if ($report != '') {
        ...
        $set['success'] = '1';
        $set['msg'] = $app_lang['report_success'];
    } else {

        $set['success'] = '0';
        $set['msg'] = 'Please add report text';
    }
    ...
    $set['redeem_id'] = $data['id'];
    $set['user_points'] = $data['user_points'];
    $set['redeem_price'] = $data['redeem_price'];
    $set['payment_mode'] = $data['payment_mode'];
    $set['bank_details'] = $data['bank_details'];
    $set['request_date'] = date('d-m-Y', strtotime($data['request_date']));

    if ($data['cust_message'] != null) {
        $set['cust_message'] = $data['cust_message'];
    } else {
        $set['cust_message'] = '';
    }
    if ($data['receipt_img'] != null && $data['status'] == 1) {
        $set['receipt_img'] =  $file_path . 'images/payment_receipt/' . $data['receipt_img'];
    } else {

        $set['receipt_img'] =  '';
    }

    $set['responce_date'] = date('d-m-Y', strtotime($data['responce_date']));
    $set['td_status'] = $data['status'];
    ...
    if ($user_id != '' || isset($user_id)) {
        ...
        $set['ANDROID_REWARDS_APP'][] = array('status' => '1', 'message' => '', 'msg' => 'User ID is required !', 'success' => '0');
    }
    ...
    if ($row['total_cnt'] < SPINNER_LIMIT) {
        ...
        $set['success'] = '1';
        $set['msg'] = $app_lang['point_save'] . ' ' . $ponints . ' points';
    } else {
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($row['status'] == '1') {
            $set['ANDROID_REWARDS_APP'] = array('status' => '1', 'message' => '', 'msg' => 'Your profile is verified..', 'success' =>
            '1');
            header('Content-Type: application/json; charset=utf-8');
            echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
            die();
            ...
            $set = array('status' => '1', 'message' => '', 'msg' => 'Verification request is sended to admin...', 'success' =>
            '1');
            header('Content-Type: application/json; charset=utf-8');
            echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
            die();
        ...
        $set = array('status' => '1', 'message' => '', 'msg' => 'Verification request is sended to admin...', 'success' => '1');
    ...
    $sql = "SELECT * FROM tbl_comments WHERE tbl_comments.`post_id`='$post_id' AND `type`='$type' ORDER BY tbl_comments.`id` DESC";
    ...
    echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    die();
} else if ($get_method['method_name'] == "app_term_condition") {
    ...
    $row['success'] = 1;
    $row['msg'] = '';
    $row['app_term_condition'] = stripslashes($data['app_term_condition']);
...
// start code updated by arjann006@gmail.com 08/01/2022
else if ($get_method['method_name'] == "sub_cat_list") {
    $jsonObj = array();

    $category_id = $get_method['cat_id'];

    $page_limit = API_PAGE_LIMIT;

    // $cat_order=API_CAT_ORDER_BY;
    $cat_order = 'sub_category_name';
    if ($get_method['page'] != null) {
        $limit = ($get_method['page'] - 1) * $page_limit;
        $query = "SELECT * FROM tbl_sub_category WHERE tbl_sub_category.`status`='1' AND tbl_sub_category.`category_id`='$category_id'  ORDER BY tbl_sub_category." . $cat_order . " LIMIT $limit, $page_limit";
    } else {
        $query = "SELECT * FROM tbl_sub_category WHERE tbl_sub_category.`status`='1' AND tbl_sub_category.`category_id`='$category_id'  ORDER BY tbl_sub_category." . $cat_order;
    }
    $sql = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
    ...
    while ($data = mysqli_fetch_assoc($sql)) {
        ...
        $row['sub_category_image'] = $file_path . 'images/' . $data['sub_category_image'];
        $row['sub_category_image_thumb'] = $file_path . 'images/thumbs/' . $data['sub_category_image'];
    ...
    echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    die();
} else if ($get_method['method_name'] == "status_by_sub_cat_id") {
    $cat_id = $get_method['cat_id'];
    $sub_cat_id = $get_method['sub_cat_id'];
    $user_id = $get_method['user_id'];
    $page_limit = API_PAGE_LIMIT;
    ...
    if (!empty($lang_ids)) {
        ...
        if($sub_cat_id == ''){
            ...
            $query = "SELECT * FROM tbl_video
    			LEFT JOIN tbl_category ON tbl_video.`cat_id`=tbl_category.`cid` 
    			WHERE tbl_video.`status`='1' AND tbl_category.`status`='1' AND tbl_video.`cat_id`='$cat_id' AND tbl_video.`sub_cat_id`='$sub_cat_id' ORDER BY tbl_video.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
        }
    }
    ...
    if (!empty($lang_ids)) {
        ...
        if($sub_cat_id == ''){
            ...
            $query = "SELECT * FROM tbl_img_status
    			LEFT JOIN tbl_category ON tbl_img_status.`cat_id`=tbl_category.`cid`
    			WHERE tbl_img_status.`status`='1' AND tbl_category.`status`='1' AND tbl_img_status.`cat_id`='$cat_id' AND tbl_img_status.`sub_cat_id`='$sub_cat_id' AND ($column) ORDER BY tbl_img_status.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
        }
    } else {
        if($sub_cat_id == ''){
                    $query = "SELECT * FROM tbl_img_status
            ...
            $query = "SELECT * FROM tbl_img_status
			LEFT JOIN tbl_category ON tbl_img_status.`cat_id`=tbl_category.`cid`
			WHERE tbl_img_status.`status`='1' AND tbl_category.`status`='1' AND tbl_img_status.`cat_id`='$cat_id' AND tbl_img_status.`sub_cat_id`='$sub_cat_id' ORDER BY tbl_img_status.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
        }
    }
    ...
    if (!empty($lang_ids)) {
        ...
        if($sub_cat_id == ''){
            ...
            $query = "SELECT * FROM tbl_quotes
    			LEFT JOIN tbl_category ON tbl_quotes.`cat_id`=tbl_category.`cid`
    			WHERE tbl_quotes.`status`='1' AND tbl_category.`status`='1' AND tbl_quotes.`cat_id`='$cat_id' AND tbl_quotes.`sub_cat_id`='$sub_cat_id' AND ($column) ORDER BY tbl_quotes.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
        }
    } else {
        if($sub_cat_id == ''){
            $query = "SELECT * FROM tbl_quotes
            ...
            $query = "SELECT * FROM tbl_quotes
    			LEFT JOIN tbl_category ON tbl_quotes.`cat_id`=tbl_category.`cid`
    			WHERE tbl_quotes.`status`='1' AND tbl_category.`status`='1' AND tbl_quotes.`cat_id`='$cat_id' AND tbl_quotes.`sub_cat_id`='$sub_cat_id' ORDER BY tbl_quotes.`id` " . $post_order_by . " LIMIT $limit, $page_limit";
        }
    }
...
// end code updated by arjann006@gmail.com 08/01/2022
else {
    $set['status'] = 0;
